...
def parse_cIP_file(path):
    """Parses a client IP file."""
    with open(path) as file: 
...
def main():
    ...
    parser.add_argument("-cf", "--client_ips_file",
                        type=str,
                        dest="client_IPs",
                        metavar="client_IPs",
                        help="load client IP addresses from file")
    ...
    c_IPs = parse_cIP_file(args.client_IPs)
    # Start the attack(s)
    ...
    try:
        ...
        #        client_IPs = [line.rstrip() for line in f]

        for i in c_IPs:
            print(i)
        
        loris = LorisClient(c_IPs)
